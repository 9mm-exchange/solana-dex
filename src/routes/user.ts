...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (!existingPendingUser) {
      ...
      // Set a timeout to delete the nonce after 30 seconds
      setTimeout(async () => {
        await PendingUser.deleteOne({ wallet });
        console.log(`Nonce for wallet ${wallet} deleted after 30 seconds.`);
      }, 30000);

      return res.status(200).send(newPendingUser);
    } else {
      return res.status(400).json({ message: "A user with this wallet already requested." });
    }
  } catch (error) {
...
router.post("/confirm", async (req, res) => {
  ...
  try {
    ...
    if (!body.isLedger) {
      ...
      console.log("nonce -----> ", foundNonce.nonce)
