...
const Withdraw: React.FC = () => {
  ...
  if (lpBalance < lpTokenAmount) {
    setErrorMessage(`Amount exceeds your LP balance of ${lpBalance}`);
  ...
  const handlePositionSelect = (position: PositionData) => {
    if (!position.token0 || !position.token1) {
      setErrorMessage("Invalid position data");
      return;
    }
  ...
  const handleWithdraw = async (
    ...
    if (!wallet.publicKey) {
      showNotification('error', 'Wallet not connected');
      return;
    }

    if (!token0Data?.address || !token1Data?.address) {
      showNotification('error', 'Invalid token data');
      return;
    }
    ...
    try {
      ...
      if (res && typeof res === 'object' && 'txid' in res) {
        showNotification('success', 'Transaction confirmed!', res.txid as string);
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    ...
                    setErrorMessage("Missing token data or amounts");
                  }
                }}
              >
                Remove Liquidity
